...
def test_code(test_case):
    ...
    W=sqrt( pow(WC_location_eval[0],2)+pow(WC_location_eval[1],2) )-0.35
    
    A = 1.500971685275908
    B = sqrt(pow(L,2)+pow(W,2))
    C = 1.25
    
    angle_a = acos((B*B+C*C-A*A)/(2*B*C))
    angle_b = acos((A*A+C*C-B*B)/(2*A*C))
    
    theta1 = atan2(WC_location_eval[1],WC_location_eval[0])
    theta2 = pi/2.0-angle_a-atan2(L,W)
    ...
    theta5 = atan2( sqrt( pow(R_3_6[0,2],2)+pow(R_3_6[2,2],2)), R_3_6[1,2] )
    ...
    your_wc = [WC_location_eval[0],WC_location_eval[1],WC_location_eval[2]] # <--- Load your calculated WC values in this array
